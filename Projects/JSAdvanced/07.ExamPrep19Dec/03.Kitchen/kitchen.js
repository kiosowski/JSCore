    ...
    makeTheOrder(meal){
        ...
        for (const key in neededProducts){
            if(neededProducts.hasOwnProperty(key)){
                if(!this.productsInStock.hasOwnProperty(key) || neededProducts[key] > this.productsInStock[key]) {
        ...
        return `Your order (${meal}) will be completet in the next 30 minutes and will cost you ${mealPrice}.`;
    }
}